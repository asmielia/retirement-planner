...
export function runCoupleProjection(inputs, strategy = "optimize") {
  ...
  for (let year = 1; year <= totalYears; year++) {
    ...
    function applyRrspTopUp(rrspWd, rrspAvail, lockedIncome, oasThreshNom, person, personAge) {
    ...
    if (youRetired || partnerRetired) {
      ...
      for (let iter = 0; iter < 10; iter++) {
        ...
        const yTopUp = youCanWithdraw ? applyRrspTopUp(yWd.rrspWd, yRrspAvail, yLocked, yOasThresh, you, youAge) : 0;
        const pTopUp = partnerCanWithdraw ? applyRrspTopUp(pWd.rrspWd, pRrspAvail, pLocked, pOasThresh, partner, partnerAge) : 0;
        ...
        const combinedTax = yTaxResult.totalTax + pTaxResult.totalTax;
        const combinedClawback = yTaxResult.oasClawback + pTaxResult.oasClawback;
        // Net = spending withdrawals + fixed income - full tax (including top-up) - clawback
      ...
      if (youCanWithdraw) {
        const extraYRrsp = applyRrspTopUp(yRrspWd, yRrspAvail, yLocked, yOasThresh, you, youAge);
        if (extraYRrsp > 0) yRrspWd += extraYRrsp;
      }
      if (partnerCanWithdraw) {
        const extraPRrsp = applyRrspTopUp(pRrspWd, pRrspAvail, pLocked, pOasThresh, partner, partnerAge);
